...
import org.d3if3127.submition1.databinding.ActivityMapsBinding
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.option_menumaps, menu)
        return true
    }
    ...
    private fun addManyMarker() {
        viewModel.getLocation.observe(this) { data ->
            data.listStory.forEach { data ->
                val latLng = LatLng(data.lat ?: 0.0, data.lon ?: 0.0) // Provide default values if lat or lon is null
                mMap.addMarker(
                    MarkerOptions()
                        .position(latLng)
                        .title(data.name)
